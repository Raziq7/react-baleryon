...
// Define a type that includes the props we want to pass to the icon
type IconProps = {
  size?: number;
  className?: string;
  onClick?: React.MouseEventHandler<SVGElement>;
  onMouseEnter?: React.MouseEventHandler<SVGElement>;
  'data-star-index'?: number | string;
}
...
interface RatingsProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  size?: number
  fill?: boolean
  Icon?: React.ReactElement<IconProps>  // Update this to specify expected props
  variant?: keyof typeof ratingVariants
  onRatingChange?: (rating: number) => void
}
...
export const CommentRatings = ({
  ...
  const handleStarClick = (event: React.MouseEvent<SVGElement>) => {
    const starIndex = parseInt(
      (event.currentTarget as SVGElement).dataset.starIndex || "0"
    )
    setCurrentRating(starIndex)
    setHoverRating(null)
    onRatingChange?.(starIndex)
  ...
  const handleStarMouseEnter = (event: React.MouseEvent<SVGElement>) => {
    const starIndex = parseInt(
      (event.currentTarget as SVGElement).dataset.starIndex || "0"
    )
    setHoverRating(starIndex)
...
interface PartialStarProps {
  ...
  size: number
  className?: string
  Icon: React.ReactElement<IconProps>  // Update this type as well
}
