...
import { ProductDetailInterface, ProductListCard } from "@/components/items/productCard";
...
function Page() {
  ...
  useEffect(() => {
    const fetchProductList = async () => {
      ...
      const response = await api.get(`/api/user/product/getProducts?${query.toString()}`);
      setProductList(response.data.products);
    };
    fetchProductList();
  }, [selectedCategories, selectedColors]); // fetch whenever filters change

  const renderCheckbox = (label: string, state: string[], setState: React.Dispatch<React.SetStateAction<string[]>>) => (
  ...
  return (
    <div className="px-4 md:px-8">
      <div className="mt-6 md:mt-10 mb-12 md:mb-18">
        <h2 className="text-2xl md:text-3xl text-center">SHOP BY CATEGORIES</h2>
        <p className="text-[#7A7879] text-center text-sm md:text-base">
      ...
      <div className="flex flex-col md:flex-row gap-5">
              ...
              <AccordionContent>
                <ol className="flex flex-col gap-2">
                  {["Red", "Blue", "Black", "Green", "White"].map(color => (
                    <li key={color}>
                      {renderCheckbox(color.toUpperCase(), selectedColors, setSelectedColors)}
                    </li>
                  ))}
                </ol>
              </AccordionContent>
        ...
        <div className="flex-1">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 justify-center items-center gap-4 md:gap-x-10 md:gap-y-16">
            {productList.map((productDetail, i) => (
              <ProductListCard key={i} {...productDetail} />
            ))}
          </div>
        </div>
      </div>
